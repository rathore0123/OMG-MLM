...
import { InvestmentHistory, LevelIncomeReport, LevelwiseTeam, MyDirect } from "../../../utils/Constant";
...
const Template: React.FC = () => {
  ...
  //centralised values
  const procedureName = "GetLevelIncome";
  const pageTitle = LevelIncomeReport;
  const landingPageTitle = "Level Income";
  const breadCrumbs = {
  ...
  const fetchExportData = useCallback(async () => {
    ...
    const res = await universalService(payload);
    return res?.data ?? res ?? [];
  }, [filterColumn, selectedLevel, searchInput, page, sortIndex, sortDirection]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const fetchGridData = useCallback(
    ...
    [page, perPage, selectedLevel, sortIndex, sortDirection, filterColumn, searchInput],
  ); // eslint-disable-line react-hooks/exhaustive-deps
