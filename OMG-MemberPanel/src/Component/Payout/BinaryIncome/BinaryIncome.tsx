...
import { BinaryIncomeReport } from "../../../utils/Constant";
...
const Template: React.FC = () => {
  ...
  //centralised values
  const procedureName = "BinaryIncomeReport";
  const pageTitle = BinaryIncomeReport;
  const landingPageTitle = "Binary Income";
  const breadCrumbs = {
      ...
      <Container fluid>
        <div className="trezo-card">
          ...
          {/* <div
            ...
            style={{ background: "rgba(99,102,241,0.07)", border: "1px solid rgba(99,102,241,0.15)", fontSize: 12 }}
          >
            <span style={{ color: "#6366f1", fontWeight: 600 }}>Binary Income Rules (Active BV only):</span>
            <span>💰 <strong>₹{dailyCapInfo.pairRate}</strong>/pair</span>
            <span>🔒 Max <strong>{dailyCapInfo.dailyCap} pairs</strong>/day</span>
            <span>📈 Daily cap: <strong>₹{dailyCapInfo.maxDaily.toLocaleString()}</strong></span>
            <span style={{ color: "#94a3b8" }}>Carry-forward applies when one leg is stronger</span>
          </div> */}
