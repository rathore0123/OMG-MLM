...
import { TransactionalLogReport } from "../../utils/Constant";
...
const Template: React.FC = () => {
  ...
  //centralised values
  const procedureName = "GetTransactionLogsReport";
  const pageTitle = TransactionalLogReport;
  const landingPageTitle = "Transaction Log";
  const breadCrumbs = {
    parent: "Home",
  ...
  const statsConfig = [
    ...
    },
    {
      ...
      showCurrency: true,
      sub: "Today Transaction",
      color: "var(--btn-bg), #4cc9f0",
      icon: "✅",
    },
    {
      key: "ThisMonthTransaction",
      title: "This Month Transaction",
      showCurrency: true,
      sub: "This Month Transaction",
  ...
  const GetStats = useCallback(async () => {
    const payload = {
      procName: procedureName,
      Para: JSON.stringify({ ActionMode: "GetStats", ClientId: ClientID }),
    };
    const res = await universalService(payload);
  ...
  return (
    <>
      {/* HEADER */}
       ...
       <Breadcrumbs mainTitle="Transaction Log Report" parent="Home" ChildName="Transaction Log"/>
      <Container fluid>
