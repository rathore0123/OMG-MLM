...
export default function DateRangeFilter({ onChange, initialRange }: Props) {
  ...
  const [popFixedPos, setPopFixedPos] = useState<React.CSSProperties>({});
  ...
  return (
    <>
      ...
      <div ref={wrapRef} className="drf-root">
        ...
        <button className="drf-trigger" onClick={openPicker} type="button">
          <FaCalendarAlt
            size={13}
            style={{ color: "#3b82f6", flexShrink: 0 }}
          />
          <span className="drf-trigger-text">{triggerLabel}</span>
          <span className={`drf-chevron ${open ? "open" : ""}`}>▾</span>
        </button>

        {/* ── DESKTOP POPUP ── */}
        {open && !isMobile && (
          <div className="drf-popup" style={popFixedPos}>
            <PresetsPanel />
