...
export default function DateRangeFilter({
  ...
  // ── NEW: portal popup positioning (viewport-fixed coords) ──
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      // NOTE: popup is now portaled to document.body, so it is no longer
      // a DOM descendant of wrapRef. We must also ignore clicks that land
      // inside the portaled popup itself, or the popup will close the
      // instant the user clicks a day/button inside it.
  ...
  // ── NEW: ref for the portaled popup (used by outside-click handler) ──
  const popupRef = useRef<HTMLDivElement>(null);

  // ── NEW: compute + keep the popup's fixed viewport position in sync ──
  const updatePopupPosition = useCallback(() => {
    if (!wrapRef.current) return;
    const rect = wrapRef.current.getBoundingClientRect();
    const flipUp = window.innerHeight - rect.bottom < 380;
    setPopPos(flipUp ? "top" : "bottom");
    if (flipUp) {
      setPopupCoords({
  ...
  useEffect(() => {
    ...
    // scrollable containers (e.g. the dashboard card), not just window
  ...
  }, [open, isMobile, updatePopupPosition]);
  ...
  `}
        ...
        {/* ── DESKTOP POPUP ── */}
        {/* NEW: portaled to document.body + position:fixed with viewport
            coords, so it can no longer be trapped behind a sticky/fixed
            app header's stacking context, and can't be clipped by any
            ancestor's overflow:hidden/auto. */}
        {open &&
          !isMobile &&
          createPortal(
            <div
      ...
      {/* ── MOBILE BOTTOM SHEET ── */}
      {/* NEW: also portaled to document.body for the same stacking-context
          safety, even though it already used position:fixed. */}
      {open &&
        isMobile &&
        createPortal(
          <>
            <div className="drf-backdrop" onClick={() => setOpen(false)} />
            <div className="drf-sheet" ref={popupRef}>
...
/* ─── CSS ─────────────────────────────────────────── */
const CSS = `
/* NOTE: z-index removed here on purpose. The popup/sheet are now portaled
   to document.body, so this element's own stacking context no longer
   determines whether the popup can appear above the app header. */
.drf-root { position: relative; width: fit-content; }
...
/* ── Desktop Popup ──
   position is now set inline (fixed + computed top/bottom/left) because
   the element is portaled to document.body. z-index is pushed well above
   any realistic app-header z-index (Tailwind dashboards commonly use
   z-40/z-50/z-[100]). */
.drf-popup {
  z-index: 9999;
