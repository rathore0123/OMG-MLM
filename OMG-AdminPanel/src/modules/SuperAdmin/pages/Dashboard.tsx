...
const Skeleton = ({ className = "" }) => (
  <div
    className={`animate-pulse bg-gray-200 dark:bg-[#172036] rounded ${className}`}
  />
);
...
const StatCard = ({
  ...
  >
    ...
    <div
      ...
      style={{
        background: `linear-gradient(90deg, ${stripColor}, ${stripColor}99)`,
      }}
    />
    <div className="trezo-card-content flex items-start justify-between gap-3">
      ...
      {/* Dual-tone icon */}
      <div
        className={`w-12 h-12 rounded-xl relative flex items-center justify-center flex-shrink-0 mt-1 ${iconBg}`}
      >
        <i
...
const Dashboard: React.FC = () => {
  ...
  // ── Parse JSON fields ──────────────────────────────────────────────────────
  const incomeBreakdown = parseJson<IncomeItem[]>(
    data?.IncomeBreakdown ?? "",
    [],
  );
  const earningsChart = parseJson<EarningsItem[]>(
    data?.EarningsChart ?? "",
    [],
  );
  const topInvestors = parseJson<TopInvestor[]>(data?.TopInvestors ?? "", []);
  const recentTxs = parseJson<RecentTx[]>(data?.RecentTransactions ?? "", []);
  const recentRegs = parseJson<RecentReg[]>(
  ...
  const earningsOpts: ApexOptions = {
    ...
    fill: {
      type: "gradient",
      gradient: { stops: [0, 90, 100], opacityFrom: 0.4, opacityTo: 0.02 },
    },
    ...
    <>
      ...
      >
        ...
        <StatCard
          ...
          prefix={cs}
          decimals={2}
          sub={`Commission ${cs}${fmt(data?.TotalCommissionWallet ?? 0)}`}
        ...
        <StatCard
          ...
          prefix={cs}
          decimals={2}
          sub={`Sponsor ${cs}${fmt(data?.TotalSponsorIncome ?? 0)}`}
          icon="payments"
          numColor="text-emerald-600 dark:text-emerald-400"
          iconColor="text-emerald-500"
          iconBg="bg-emerald-50 dark:bg-emerald-900/20"
          stripColor="#10b981"
          onClick={go("/superadmin/commission/sponsor-income-report")}
        />
        <StatCard
          ...
          <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
            <div className="trezo-card-header mb-[20px] md:mb-[25px] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 !mb-0">
                    Last 30 days
                  </p>
                </div>
              </div>
              ...
              {Chart && earningsChart.length > 0 ? (
                <Chart
                  ...
                  series={[
                    {
                      name: "Earnings",
                      data: earningsChart.map((e) => e.earnings),
                    },
                    {
                      name: "Withdrawals",
                      data: earningsChart.map((e) => e.withdrawals),
                    },
                  ]}
                  type="area"
                  height={300}
                ...
                <div className="h-[300px] flex items-center justify-center text-sm text-gray-400">
                  {!Chart
                    ? "Loading chart…"
                    : "No transaction data for last 30 days"}
                </div>
              )}
            </div>
            <ul className="text-center mt-3 border-t border-gray-100 dark:border-[#172036] pt-3">
              {[
                {
                  lbl: "Total Earnings",
                  val: `${cs}${fmt(data?.TotalEarning ?? 0)}`,
                  color: "text-[#605DFF]",
                },
                {
                  lbl: "Total Withdrawals",
                  val: `${cs}${fmt(data?.TotalWithdrawals ?? 0)}`,
                  color: "text-purple-500",
                },
              ].map(({ lbl, val, color }) => (
                <li key={lbl} className="inline-block mx-[13px]">
                  <div className="flex items-center gap-2">
                    <span className={`font-semibold text-[18px] ${color}`}>
                      {val}
                    </span>
          ...
          <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-xl border border-gray-100 dark:border-gray-800">
            <div className="trezo-card-header flex items-center justify-between mb-[20px] md:mb-[25px]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h5 className="!mb-0 font-bold text-gray-800 dark:text-white tracking-tight">
                    Pending Stats
                  </h5>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 !mb-0">
                    Requires action
            ...
            <div className="space-y-4">
              {(
                [
                  ...
                  {
                    label: "Approved",
                    amount: data?.ApprovedWithdrawalAmount ?? 0,
                    count: data?.ApprovedWithdrawalCount ?? 0,
                  ...
                  {
                    label: "Rejected",
                    amount: data?.RejectedWithdrawalAmount ?? 0,
                    count: data?.RejectedWithdrawalCount ?? 0,
                    bg: "bg-rose-50    dark:bg-rose-900/20",
                    border: "border-rose-200    dark:border-rose-800",
                    num: "text-rose-500",
                  },
                ] as const
              ).map(({ label, amount, count, bg, border, num }, i) => (
                <motion.div
                  key={label}
                ...
                >
                  <div>
                    ...
                    <p
                      className={`text-lg font-bold ${num} !mb-0 leading-tight mt-0.5`}
                    >
                      {cs}
                      {fmt(amount)}
                    </p>
                  ...
                  <div className="text-right">
                    <span className={`text-2xl font-extrabold ${num}`}>
                      {count}
                    </span>
        ...
        >
          <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md h-[94%]">
            ...
            <div className="space-y-3">
              ...
              ).map(({ label, val, icon, accent, bar, badge }, i) => {
                ...
                <tbody className="text-black dark:text-white">
                  ...
                  {topInvestors.length === 0 && (
                    <tr>
                      ...
                      >
                        <i className="material-symbols-outlined text-[36px] block mb-1">
                          leaderboard
                        </i>
                        No investors found
                      </td>
                  ...
                  <thead>
                    <tr>
                      {[
                        ...
                        <th
                          key={h}
                          className="font-medium ltr:text-left px-[20px] py-[11px] md:ltr:first:pl-[25px] md:rtl:first:pr-[25px] bg-primary-50 dark:bg-[#15203c] whitespace-nowrap text-black dark:text-white text-sm"
                        >
                          {h}
                        </th>
                      ))}
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5 !mb-0">
                  ...
                  <span className="font-semibold text-black dark:text-white">
                    {cs}
                    {fmt(data?.TotalIncomeDistributed ?? 0)}
                  </span>
                </p>
              </div>
            </div>
            <div className="space-y-6">
              {incomeBreakdown.filter(item => !item.IncomeType.toLowerCase().includes("roi")).map((item, i) => (
                <motion.div
                  key={item.IncomeType}
                  ...
                  transition={{ delay: 0.2 + i * 0.1, duration: 0.3 }}
                  onClick={() => {
                    const r = incomeRoutes[item.IncomeType];
                    if (r) navigate(r);
                  }}
                  className={
                    incomeRoutes[item.IncomeType] ? "cursor-pointer group" : ""
                  }
                >
                  ...
                  {recentTxs.map((tx, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="ltr:text-left whitespace-nowrap px-[20px] py-[12px] border-b border-gray-100 dark:border-[#172036]">
                          <span
                            className={`px-[8px] py-[3px] inline-flex items-center gap-1 rounded-sm font-semibold text-xs ${
                              isCr
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${isCr ? "bg-emerald-500" : "bg-rose-500"}`}
                            />
                            {tx.TransType?.trim()}
                          </span>
                        </td>
                        <td className="ltr:text-left whitespace-nowrap px-[20px] py-[12px] border-b border-gray-100 dark:border-[#172036]">
                          <span
                            className={`font-bold text-sm ${isCr ? "text-emerald-500" : "text-rose-500"}`}
                          >
                            {isCr ? "+" : "-"}
                            {cs}
