...
const DetailModal: React.FC<DetailModalProps> = ({ open, onClose, details }) => {
  ...
  const rows = [
    { lbl: "Top Up Amount", val: `${currency.symbol}${details.TopUpAmount ?? "—"}`, gold: true },
    { lbl: "Designation", val: details.RankName ?? "—" },
    { lbl: "ID Activation Date", val: details.IDActivationDate ?? "—" },
    { lbl: "Today Business", val: `${currency.symbol}${details.TodayBusiness ?? "—"}`, gold: true },
    { lbl: "Stronger Business", val: `${currency.symbol}${details.LeftBusiness ?? "—"}` },
    { lbl: "Weaker Business", val: `${currency.symbol}${details.RightBusiness ?? "—"}` },
    { lbl: "Total Business", val: `${currency.symbol}${details.TotalBusiness ?? "—"}`, gold: true },
  ];
