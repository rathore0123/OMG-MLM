...
function injectStyles() {
  ...
  style.textContent = `
    ...
    /* ── FIX: card border reflects paid/unpaid status ──
       Two-class selectors just like .root above, but the :hover variants
       use three classes so they win specificity over the generic
       ".org-node-card:hover" cyan-border rule — the border should stay
       green/red on hover, not flip to cyan. Placed after .root so
       paid/unpaid status is the visually dominant signal even on the
       root node (root keeps its gold avatar ring/username color, only
       the border/glow switches to green or red). */
    .org-node-card.paid {
      border-color: var(--ot-green);
      box-shadow: 0 0 0 1px var(--ot-green), 0 8px 24px rgba(22,163,74,0.15);
    }
    .org-node-card.paid:hover {
      border-color: var(--ot-green);
      box-shadow: 0 0 0 2px var(--ot-green), 0 8px 24px rgba(22,163,74,0.2);
    }
    .org-node-card.unpaid {
      border-color: var(--ot-red);
      box-shadow: 0 0 0 1px var(--ot-red), 0 8px 24px rgba(220,38,38,0.15);
    }
    .org-node-card.unpaid:hover {
      border-color: var(--ot-red);
      box-shadow: 0 0 0 2px var(--ot-red), 0 8px 24px rgba(220,38,38,0.2);
    }
