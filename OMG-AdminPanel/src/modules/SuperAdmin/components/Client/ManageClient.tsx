...
const Template: React.FC = () => {
          ...
          cell: (row: any) => {
            ...
            return (
              <div className="flex gap-2 items-center">
                ...
                />
                <PermissionAwareTooltip allowed={true} allowedText="Login as Member">
                  <button
                    className="text-blue-500 hover:text-blue-700 transition-colors"
                    onClick={() => handleLoginAsMember(row)}
                  >
                    <i className="material-symbols-outlined !text-md">login</i>
                  </button>
  ...
  const handleLoginAsMember = async (row: any) => {
    try {
      const res = await universalService({
        procName: "AdminGetMemberCredentials",
        Para: JSON.stringify({ ClientId: row.ClientId }),
      });
      const result = res?.data ?? res;
      const cred = Array.isArray(result) ? result[0] : result;
      if (!cred?.UserName || !cred?.PlainPassword) {
        alert("Member credentials not found.");
        return;
      }
      // env Variable added 
      const url = `${import.meta.env.VITE_MEMBER_PANEL_URL}?username=${encodeURIComponent(cred.UserName)}&password=${encodeURIComponent(cred.PlainPassword)}`;
      window.open(url, "_blank");
    } catch {
      alert("Failed to login as member.");
    }
  };
