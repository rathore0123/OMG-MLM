...
const MemberWalletsElegant: React.FC = () => {
  ...
  const DEFAULT_LOGO_URL = `${import.meta.env.BASE_URL}/images/default_user.png`.replace(/\/{2,}/g, "/");
  // GetClientDetails (USP_Client) returns ISNULL(ClientLogo,'default.png') server-side —
  // that placeholder filename isn't guaranteed to exist on the uploads server, so skip
  // the network request entirely rather than waiting for it to 404 into onError.
  const isPlaceholderLogo = (logo?: string) => !logo || /^default\.(png|jpe?g)$/i.test(logo);
            ...
            <div className="flex items-center justify-between gap-4 pl-4 border-l border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-3">
                <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 ring-2 ring-gray-100 dark:ring-gray-700">
                  <img
                    src={
                      isPlaceholderLogo(memberDetails?.Logo)
                        ? DEFAULT_LOGO_URL
                        : `${imageBaseUrl}${memberDetails?.Logo}`
                    }
                    alt="Client Logo"
                    className="w-full h-full object-cover"
                    onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                      // Covers a genuinely broken (non-placeholder) uploaded logo —
                      // clear the handler so the local asset can't loop either.
