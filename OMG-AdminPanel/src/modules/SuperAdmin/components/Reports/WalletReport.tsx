...
// Sentinel ID used to mark the synthetic "Total" row appended to table data
...
const exportColumns = [
  ...
  { key: "ProductWallet", label: "Deposit Wallet" },
];
...
const WalletReport: React.FC = () => {
  ...
  // ✅ Totals for the currently loaded page (server-side pagination means
  // this reflects only the rows on screen, not the full filtered result set)
  const pageTotals = useMemo(
    () =>
      data.reduce(
        (acc, row) => {
          acc.commission += Number(row.CommissionWallet ?? 0);
          acc.deposit += Number(row.ProductWallet ?? 0);
          return acc;
        },
  ...
  // ✅ Append a synthetic "Total" row to the data fed into DataTable so the
  // totals line up under the Commission Wallet / Deposit Wallet columns
  // using the exact same grid the rest of the rows use.
  const tableData = useMemo<WalletRow[]>(() => {
    if (data.length === 0) return data;
    const totalRow: WalletRow = {
      ClientId: TOTAL_ROW_ID,
      MemberName: "Total",
      UserName: "",
      ClientLogo: null,
      Status: "",
      WalletAmount: 0,
      ROIWallet: 0,
      CommissionWallet: pageTotals.commission,
      ProductWallet: pageTotals.deposit,
      TotalRecords: 0,
    };
  ...
  const columns: TableColumn<WalletRow>[] = useMemo(
    () => [
      ...
      // },
      {
        ...
        cell: (r) => (
          <span
            className={r.ClientId === TOTAL_ROW_ID ? "font-bold" : undefined}
          >
            {fmt(r.CommissionWallet)}
          </span>
        ),
      },
          ...
          <div className="trezo-card-content bg-white dark:bg-[#0f172a] text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <DataTable
              ...
              conditionalRowStyles={[
                {
                  when: (row: WalletRow) => row.ClientId === TOTAL_ROW_ID,
                  style: {
